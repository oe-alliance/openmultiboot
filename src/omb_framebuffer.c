...
int omb_read_screen_info()
{
	if (ioctl(omb_fb_fd, FBIOGET_FSCREENINFO, &omb_fix_screen_info) == -1) {
		omb_log(LOG_ERROR, "%-33s: cannot read fixed information", __FUNCTION__);
		return OMB_ERROR;
	}

	if (ioctl(omb_fb_fd, FBIOGET_VSCREENINFO, &omb_var_screen_info) == -1) {
		omb_log(LOG_ERROR, "%-33s: cannot read variable information", __FUNCTION__);
		return OMB_ERROR;
	}

	omb_log(LOG_DEBUG, "%-33s: current mode is %dx%d, %dbpp, stride %d", __FUNCTION__,
...
int omb_set_screen_info(int width, int height, int bpp)
{
	...
	if ((omb_var_screen_info.xres != width) && (omb_var_screen_info.yres != height) && (omb_var_screen_info.bits_per_pixel != bpp)) {
		omb_log(LOG_ERROR, "%-33s: cannot set variable information: got %dx%dx%d instead of %dx%dx%d", __FUNCTION__,
...
int omb_map_framebuffer()
{
	...
	omb_log(LOG_DEBUG, "%-33s: the framebuffer device was mapped to memory successfully", __FUNCTION__);
...
int omb_open_framebuffer()
{
	...
	if (omb_fb_fd == -1) {
		omb_log(LOG_ERROR, "%-33s: cannot open framebuffer device", __FUNCTION__);
		return OMB_ERROR;
	}
	omb_log(LOG_DEBUG, "%-33s: the framebuffer device was opened successfully", __FUNCTION__);
