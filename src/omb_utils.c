...
{
	omb_log(LOG_DEBUG, "%-33s: ", __FUNCTION__);
...
{
	if (item == NULL || strcmp(item->identifier, "flash") == 0) {
		omb_log(LOG_DEBUG, "%-33s: execl %s %s", __FUNCTION__, OMB_SYSVINIT_BIN, args);
		...
		omb_log(LOG_DEBUG, "%-33s: execl %s %s %s %s", __FUNCTION__, OMB_CHROOT_BIN, path, OMB_INIT_BIN, args);
