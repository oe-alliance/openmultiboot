...
int omb_lcd_open()
{
	...
	if (omb_lcd_fd == -1) {
		omb_log(LOG_ERROR, "%-33s: cannot open lcd device", __FUNCTION__);
		return OMB_ERROR;
	}
	...
	if (ioctl(omb_lcd_fd, LCD_IOCTL_ASC_MODE, &tmp)) {
		omb_log(LOG_ERROR, "%-33s: failed to set lcd bin mode", __FUNCTION__);
#ifndef OMB_DREAMBOX
		return OMB_ERROR;
#endif
	}
	...
	if (omb_lcd_width == 0) {
		omb_log(LOG_ERROR, "%-33s: cannot read lcd x resolution", __FUNCTION__);
		return OMB_ERROR;
	}
	
	omb_lcd_height = omb_lcd_read_value(OMB_LCD_YRES);
	if (omb_lcd_height == 0) {
		omb_log(LOG_ERROR, "%-33s: cannot read lcd y resolution", __FUNCTION__);
		return OMB_ERROR;
	}
	omb_lcd_bpp = omb_lcd_read_value(OMB_LCD_BPP);
	if (omb_lcd_bpp == 0) {
		omb_log(LOG_ERROR, "%-33s: cannot read lcd bpp", __FUNCTION__);
		return OMB_ERROR;
	}
	...
	omb_log(LOG_DEBUG, "%-33s: current lcd is %dx%d, %dbpp, stride %d", __FUNCTION__, omb_lcd_width, omb_lcd_height, omb_lcd_bpp, omb_lcd_stride);
