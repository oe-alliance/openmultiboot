...
int omb_init_freetype()
{
	...
	if (FT_New_Memory_Face(omb_freetype_library, omb_segoe_ui_font, omb_segoe_ui_font_length, 0, &omb_freetype_face) != 0) {
		omb_log(LOG_ERROR, "cannot open base font");
		return OMB_ERROR;
	}

	if (FT_New_Memory_Face(omb_freetype_library, omb_icomoon_font, omb_icomoon_font_length, 0, &omb_freetype_symbols_face) != 0) {
